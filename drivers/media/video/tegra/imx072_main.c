...
static int Imx072_RESET=TEGRA_GPIO_PD2;
static int Imx072_PWRDN =TEGRA_GPIO_PBB5;
static int Imx072R_RESET = TEGRA_GPIO_PT4;
static int Imx072R_PWRDN = TEGRA_GPIO_PT3;
...
//static void imx072_left_cam_pin_set();
//static void imx072_right_cam_pin_set();
//static int imx072_power_on();
...
//s32 i2c_write_block_data(client, u8 addr_flags, 1, addr, &val)
	do {
//		pr_info("I2C Transfer start Addr = %x\n", addr);
		err = i2c_transfer(client->adapter, &msg, 1);
//		pr_info("I2C Transfer end Addr = %x\n", addr);
		if (err >0 )
			return 0;
		retry++;
...
static void imx072_left_cam_pin_set(void)
{
	gpio_set_value(Imx072_RESET,1);
	gpio_set_value(Imx072_PWRDN,1);
	mdelay(5);
}
static void imx072_right_cam_pin_set(void)
{
...
static int imx072_power_on(void)
{
	...
	switch(camera_mode){
		case Main:
			imx072_left_cam_pin_set();
			break;
		case Stereo:
			imx072_left_cam_pin_set();
			imx072_right_cam_pin_set();
			break;
		case LeftOnly:
			imx072_left_cam_pin_set();
			break;
			...
			const struct i2c_device_id *id)
{
	...
	gpio_request(Imx072_RESET, "Cam_Reset");
	gpio_request(Imx072_PWRDN, "Cam_Power_Down");
	tegra_gpio_enable(Imx072_RESET);
	tegra_gpio_enable(Imx072_PWRDN);
	gpio_direction_output(Imx072_RESET,0);
	gpio_direction_output(Imx072_PWRDN,0);
...
//	int err;
	...
	gpio_request(Imx072R_RESET, "Right_Cam_Reset");
	gpio_request(Imx072R_PWRDN, "Right_Cam_Power_Down");
	tegra_gpio_enable(Imx072R_RESET);
	tegra_gpio_enable(Imx072R_PWRDN);
	gpio_direction_output(Imx072R_RESET,0);
	gpio_direction_output(Imx072R_PWRDN,0);
...
static const struct i2c_device_id imx072R_id[] = {
	{ "imx072R", 0 },
	{ },
};
...
static int __init imx072_init(void)
{
	pr_info("imx072 sensor driver loading\n");
	 i2c_add_driver(&imx072_i2c_driver);
