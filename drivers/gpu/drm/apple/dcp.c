...
static int dcp_dptx_connect(struct apple_dcp *dcp, u32 port, bool oob)
{
	int ret = 0;
...
int dcp_dptx_connect_oob(struct platform_device *pdev, u32 port)
{
	...
	guard(mutex)(&dcp->hpd_deferred_mutex);
	dcp->oob_port = port;
	dcp->hpd_state = DCP_HPD_OOB_CONNECTED;
...
int dcp_dptx_disconnect_oob(struct platform_device *pdev, u32 port)
{
	...
	guard(mutex)(&dcp->hpd_deferred_mutex);
	dcp->oob_port = port;
	dcp->hpd_state = DCP_HPD_OOB_DISCONNECTED;

	if (!delayed_work_pending(&dcp->hpd_wq))
		schedule_delayed_work(&dcp->hpd_wq, msecs_to_jiffies(200));
