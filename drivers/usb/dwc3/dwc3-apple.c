 ...
 * don't work or come with a 5 seconds delay.
 */
...
static void dwc3_apple_setup_cio(struct dwc3_apple *appledwc)
{
	...
	dwc3_apple_writel(appledwc, APPLE_DWC3_CIO_BW_NGT_OFFSET, APPLE_DWC3_CIO_BW_NGT_OFFSET_VALUE);

	link_timer = dwc3_apple_readl(appledwc, APPLE_DWC3_CIO_LINK_TIMER);
	link_timer &= ~APPLE_DWC3_CIO_PENDING_HP_TIMER;
	link_timer |=
		FIELD_PREP(APPLE_DWC3_CIO_PENDING_HP_TIMER, APPLE_DWC3_CIO_PENDING_HP_TIMER_VALUE);
	dwc3_apple_writel(appledwc, APPLE_DWC3_CIO_LINK_TIMER, link_timer);
}
...
static int dwc3_apple_exit(struct dwc3_apple *appledwc)
{
	...
	/* Nothing to do if no cable has ever been connected since boot */
	...
	/*
	 * It's possible to completely break the dwc3 controller by switching it to device mode and
	 * unplugging the cable at just the wrong time. If this happens dwc3 behaves as if no device
	 * is connected. CORESOFTRESET will also never clear after it has been set. The only
	 * workaround is to trigger a hard reset of the entire core with its external reset line.
	 */
...
static int dwc3_apple_core_probe(struct dwc3_apple *appledwc)
{
	...
	lockdep_assert_held(&appledwc->lock);
	WARN_ON(appledwc->core_probe_done);
...
static int dwc3_apple_init(struct dwc3_apple *appledwc, enum dwc3_apple_mode mode)
{
	...
	case DWC3_APPLE_HOST:
		...
		/*
		 * The Type-C PHY on this platform can only be configured correctly for host mode if
		 * SUSPHY is enabled.
		 */
		dwc3_enable_susphy(&appledwc->dwc, true);
	...
	case DWC3_APPLE_DEVICE:
		...
		/* Unlike host mode, we do not need to enable SUSPHY here. */
...
static int dwc3_usb_role_switch_set(struct usb_role_switch *sw, enum usb_role role)
{
	 ...
	 * is kept online. We thus tear all of dwc3 down here and re-initialize it every time we
	 * get a plug change (or even mode change) event.
	 */
...
static int dwc3_apple_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(appledwc->resets))
		return dev_err_probe(&pdev->dev, PTR_ERR(appledwc->resets), "Failed to get resets\n");
