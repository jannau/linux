				      ...
				      struct apple_dart_master_cfg *cfg)
{
	...
	dart_domain->mask = DMA_BIT_MASK(ias);
			       ...
			       const struct of_phandle_args *args)
{
	...
	if (args->args_count == 5) {
		...
		dma_addr_t size = ((dma_addr_t)args->args[3] << 32) | args->args[4];
		dma_max = dma_min + size - 1;
		/*
	...
	if (!cfg) {
		...
		if (dma_min >= cfg->dma_max || cfg->dma_min >= dma_max) {
			dev_err(dev, "non-overlapping DMA windows: %pad..%pad, "
				"%pad..%pad\n", &dma_min, &dma_max,
