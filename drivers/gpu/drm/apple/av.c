...
int dcp_audiosrv_osobject_call(struct apple_epic_service *service, u16 group, u32 command,
			       void *output, size_t output_maxsize, size_t *output_size)
{
	struct {
		__le64 max_size;
		u8 _pad1[24];
...
int avep_init(struct apple_dcp *dcp)
{
	...
	dcp->avep->debugfs_root = \
			debugfs_create_dir("avep", dcp->debugfs_root);
