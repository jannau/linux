...
#define PIPEHANDLER_AON_GENDWC3_RESET_N BIT(0)
				    ...
				    unsigned long id)
{
	...
	clear32(atcphy->regs.pipehandler + PIPEHANDLER_AON_GEN,
		PIPEHANDLER_AON_GENDWC3_RESET_N);
			  ...
			  struct typec_mux_state *state)
{
	 ...
	 */
	if (atcphy->dwc3_online && state->mode == TYPEC_STATE_SAFE) {
	...
	case APPLE_ATCPHY_MODE_USB3:
	case APPLE_ATCPHY_MODE_USB4:
		atcphy_cio_configure(atcphy, target_mode);
		break;
	default:
